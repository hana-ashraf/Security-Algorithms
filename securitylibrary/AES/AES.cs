...
{0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00 }};
        ...
        void implement_key_expansion() // round key -->bn2sm el matrix column column (a5r step)
        {
            int R_ind = 0;
            ...
            int i = 4; // bnbtdy matrix gdeda
            while (i < 44)
            {
                int k = 0;
                while (k < 4)
                {
                    first[k] = key_expansion[i - 1, k]; //bngeb el column mn el old key
                    second[k] = key_expansion[i - 4, k];//bngeb el column mn el new key bs lly 2blo
                    if (R_ind < 10)
                        third[k] = Rcon[k, R_ind];//column el rcon
                    k++;
                }
                if (i % 4 == 0)
                {
                    ...
                    fourth = xor(first, second, third, 1);//gneration of new key
                }
                else
        ...
        byte[,] mixCols(byte[,] shiftedmatrix)
        {
            ...
            for (byte i = 0; i < 4; i++)
            {
                for (byte j = 0; j < 4; j++)
                {
                    result_matrix[i, j] = find_rijandel_mix_columns_value(i, j, shiftedmatrix);
                }
            }
        ...
        public byte find_rijandel_mix_columns_value(byte row, byte col, byte[,] A)
        {
            ...
            for (int i = 0; i < 4; i++)
            {
                ...
                else if (rijandel_matrix[row, i] == 2)
                {
                    if ((A[i, col] & (1 << 7)) != 0)
                        MSB_Set = true;
        ...
        public override string Encrypt(string plainText, string key) 
        {
            ...
            int k = 2;
            for (int j = 0; j < 4; j++) // convert plain to hexa decimal
            {
                for (int i = 0; i < 4; i++)
                {
            ...
            int k1 = 2;
            for (int j = 0; j < 4; j++) //convert key to hexa decimal
            {
                for (int i = 0; i < 4; i++)
                {
            ...
            for (int i = 0; i < 4; i++) //byn2l fe matrix tanya
                for (int j = 0; j < 4; j++)
        ...
        //function used to sub_bytes operation 
        ...
        public static string[] XOORfun(string[] suba4, string[] a1, byte[,] Rcon)
        {
            ...
            for (int i = 0; i < 4; i++)
            {
                ii = a11[i] ^ a4[i] ^ Rcon[0, i];

                Newarray[i] = Convert.ToString(ii, toBase: 16);
                if (Newarray[i].Length == 1)
                {
                    Newarray[i] = "0" + Convert.ToString(ii, toBase: 16);
                }
            }
        ...
        {
            string[] ro_overrid = new string[4];
            for (int i = 0; i < 4; i++)
            {
                ro_overrid[i] = w[i];
            }
            string st = ro_overrid[0];
            ro_overrid[0] = ro_overrid[1];
            ro_overrid[1] = ro_overrid[2];
            ro_overrid[2] = ro_overrid[3];
            ro_overrid[3] = st;
            return ro_overrid;
        }
